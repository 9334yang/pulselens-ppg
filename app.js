...
async function start(){
  try{
    ...
    sampler.width=64;sampler.height=48;samples=[];times=[];running=true;startAt=performance.now();lastSample=0;
    $('#cameraPlaceholder').hidden=true;$('.camera-card').classList.add('measuring');$('#liveBadge').textContent='測量中';
    $('#startBtn').disabled=true;$('#stopBtn').disabled=false;setStatus('請將指腹覆蓋鏡頭並保持穩定');loop();
  }catch(e){setStatus(e.name==='NotAllowedError'?'相機權限遭拒，請在瀏覽器設定中允許':'無法啟用相機：'+e.message)}
}
function stop(){running=false;cancelAnimationFrame(raf);stream?.getTracks().forEach(t=>t.stop());stream=null;video.srcObject=null;$('.camera-card').classList.remove('measuring');$('#cameraPlaceholder').hidden=false;$('#liveBadge').textContent='待機';$('#startBtn').disabled=false;$('#stopBtn').disabled=true;setStatus('測量已停止')}
...
$('#startBtn').addEventListener('click',start);$('#stopBtn').addEventListener('click',stop);addEventListener('pagehide',stop);
